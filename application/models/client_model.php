...
// For Client Tabele
 ...
 //Inserting in Table(Client) of Database(travel) 
       $this->db->insert('00000_mst_client', $client); 
    ...
    $aColumns = array( TABLE_ONE . 'client_id', TABLE_ONE . 'client_name', TABLE_ONE . 'client_sht_name', TABLE_ONE . 'category_id', TABLE_ONE . 'salesperson_id', TABLE_ONE . 'salesperson_commission', TABLE_ONE . 'pay_terms_id', TABLE_ONE . 'credit_limit', TABLE_ONE . 'credit_term_desc', TABLE_ONE . 'hotel_id', TABLE_ONE . 'phone_list', TABLE_ONE . 'fax_list', TABLE_ONE . 'country_id', TABLE_ONE . 'state_id', TABLE_ONE . 'city_id', TABLE_ONE . 'address', TABLE_ONE . 'gl_ac_no', TABLE_ONE . 'email_id', TABLE_ONE . 'website', TABLE_ONE . 'note', TABLE_ONE . 'vat_no', TABLE_ONE . 'ntn_no', TABLE_ONE . 'gst_no', TABLE_ONE . 'branch_id', TABLE_ONE . 'inactive_flag', TABLE_ONE . 'attachement_detail', TABLE_TWO . 'pay_term_desc', TABLE_THREE . 'hotel_name', TABLE_FOUR . 'category_desc', TABLE_FIVE . 'salesperson_name', TABLE_SIX . 'country_name', TABLE_SEVEN . 'state_name', TABLE_EIGHT . 'city_name', TABLE_NINE . 'branch_name');

    //write column name only which has TBALE_ONE const with it.
     $bColumns = array( TABLE_ONE . 'client_id', TABLE_ONE . 'client_name', TABLE_ONE . 'client_sht_name', TABLE_ONE . 'category_id', TABLE_ONE . 'salesperson_id', TABLE_ONE . 'salesperson_commission', TABLE_ONE . 'pay_terms_id', TABLE_ONE . 'credit_limit', TABLE_ONE . 'credit_term_desc', TABLE_ONE . 'hotel_id', TABLE_ONE . 'phone_list', TABLE_ONE . 'fax_list', TABLE_ONE . 'country_id', TABLE_ONE . 'state_id', TABLE_ONE . 'city_id', TABLE_ONE . 'address', TABLE_ONE . 'gl_ac_no', TABLE_ONE . 'email_id', TABLE_ONE . 'website', TABLE_ONE . 'note', TABLE_ONE . 'vat_no', TABLE_ONE . 'ntn_no', TABLE_ONE . 'gst_no', TABLE_ONE . 'branch_id', TABLE_ONE . 'inactive_flag', TABLE_ONE . 'attachement_detail');
    // Indexed column (used for fast and accurate table cardinality)
    $sIndexColumn = $aColumns[0];
    // DB table to use
    $sTable = "00000_mst_client";
    ...
    $sQuery = "
        ...
        JOIN ".$this->db->dbprefix('salesperson')." ".ALIAS_FIVE." ON ".ALIAS_FIVE.".salesperson_id = ".ALIAS_ONE.".salesperson_id
        ...
        for ($i=0; $i<count($bColumns); $i++) {
            ...
            if ($bColumns[$i] == "inactive_flag") {
                ...
                foreach ($attachement_detail as $value) {
                    $v = $value->file_upload;
                    $html .= '<a target="_blank" href="'. base_url('assets/client_files/'.$v).'" class="attachement_detail">'.$v.'</a><br>';
                }
                }
                $row[] = $html;
            ...
            }elseif ($bColumns[$i] == 'salesperson_id') {
                $detail = $this->saleman_model->getsalemanBysalemanId($aRow[$bColumns[$i]]);
                $row[] = $detail->salesperson_name;
         ...
         try {
                ...
                if(isset($_FILES['file_name']['name'])){
                    for ($i=0; $i < count($_FILES['file_name']['name']); $i++) { 
                        ...
                        $orignal_file_name = $this->input->post('client_name').rand(1111,9999).'_'.rand(1111,9999).'.'.$file_type;
                    ...
                    $clients_file = $this->multiple_upload();

                   
                    for ($i=0; $i < count($clients_file); $i++) { 
                        array_push($client_files_details, array(
                            'file_description' => $this->input->post('file_description')[$i],
                            'file_upload' =>  $clients_file[$i]
                        ));
                    }
                }
            ...
            $client = array(
                ...
                'client_sht_name' => $this->input->post('client_sht_name'),
                'salesperson_id' => ($this->input->post('salesperson') != null ? $this->input->post('salesperson') : null),
                'salesperson_commission' => $this->input->post('salesperson_commission'),
                ...
                'attachement_detail' => json_encode($client_files_details)
            );
                }
    ...
    public function editclient(){
        ...
        $query = $this->db->get('00000_mst_client');
        if($query->num_rows() > 0){
            return $query->row();
        }else{
            return false;
    ...
    if(empty($config))
    {
        $config['upload_path'] = $this->config->config['upload_path'] . '/client_files';
    ...
    public function updateclient(){
         try {
            ...
            $client_files_details = array();
                   ...
                   /* echo "<pre>";
                    print_r($this->input->post('file_name_hidden'));die;
            ...
            $dataclient = array(
                ...
                'attachement_detail' => json_encode($client_files_details)
                 ...
                 $this->db->where('client_id', $id);
            $this->db->update('00000_mst_client', $dataclient);
            return true;
    ...
    $id = $this->input->get('client_id');
    $this->db->where('client_id', $id);
    $this->db->delete('00000_mst_client');
    if($this->db->affected_rows() > 0){
      return true;
    }else{
      return false;
