...
// For Add Hotel Tabele
 function add_hotel($data){
 //Inserting in Table(Hotel) of Database(travel) 
       $this->db->insert('00000_mst_hotels', $data);   
    }


// Fetch hotel in Client Table
    function fetch_hotel()
 {
  $this->db->order_by("hotel_name", "ASC");
  $this->db->where("inactive_flag", ACTIVE);
  $query = $this->db->get("00000_mst_hotels");
  return $query->result();
 }

// This function calls in Client As Show Hotel Details detail in Client model
             function gethotelByhotelId($hotel_id){
                $this->db->select('hotel_name');
                $this->db->from('00000_mst_hotels');
    ...
    $aColumns = array( TABLE_ONE . 'hotel_id', TABLE_ONE . 'hotel_name', TABLE_ONE . 'category', TABLE_ONE . 'phone_list', TABLE_ONE . 'fax_list', TABLE_ONE . 'country_id', TABLE_ONE . 'state_id', TABLE_ONE . 'city_id', TABLE_ONE . 'address', TABLE_ONE . 'gl_ac_no', TABLE_ONE . 'alot_gl_ac_no', TABLE_ONE . 'hot_email_id', TABLE_ONE . 'hot_website', TABLE_ONE . 'vat_no', TABLE_ONE . 'gazt_no', TABLE_ONE . 'ntn_no', TABLE_ONE . 'gst_no', TABLE_ONE . 'inactive_flag', TABLE_ONE . 'attachement_detail', TABLE_SIX . 'country_name', TABLE_SEVEN . 'state_name', TABLE_EIGHT . 'city_name');

    //write column name only which has TABLE_ONE const with it.
     $bColumns = array( TABLE_ONE . 'hotel_id', TABLE_ONE . 'hotel_name', TABLE_ONE . 'category', TABLE_ONE . 'phone_list', TABLE_ONE . 'fax_list', TABLE_ONE . 'country_id', TABLE_ONE . 'state_id', TABLE_ONE . 'city_id', TABLE_ONE . 'address', TABLE_ONE . 'gl_ac_no', TABLE_ONE . 'alot_gl_ac_no', TABLE_ONE . 'hot_email_id', TABLE_ONE . 'hot_website', TABLE_ONE . 'vat_no', TABLE_ONE . 'gazt_no', TABLE_ONE . 'ntn_no', TABLE_ONE . 'gst_no', TABLE_ONE . 'inactive_flag', TABLE_ONE . 'attachement_detail');
    // Indexed column (used for fast and accurate table cardinality)
    $sIndexColumn = $aColumns[0];
    // DB table to use
    $sTable = "00000_mst_hotels";
    ...
    foreach ($rResult->result_array() as $aRow) {
        ...
        for ($i=0; $i<count($bColumns); $i++) {
            ...
            if ($bColumns[$i] == "inactive_flag") {
                ...
                $html = '';
                if (isset($phone_list) && count($phone_list) > 0) {
                ...
                $html = '';
                if (isset($fax_list) && count($fax_list) > 0) {
                     foreach ($fax_list as $value) {
                         $html .= '<span class="fax_list">'.$value.'</span><br>';
                     }
                 } $row[] = $html;
            }elseif ($bColumns[$i] == 'attachement_detail') {
                ...
                if (isset($attachement_detail) && count($attachement_detail) > 0) {
                    foreach ($attachement_detail as $value) {
                        $v = $value->file_upload;
                        $html .= '<a target="_blank" href="'.base_url('assets/hotel_files/'.$v).'">'.$v.'</a><br>';
                    }
                }$row[] = $html;
            }
    ...
    /*public function addhotel(){
        ...
        $this->db->insert('00000_mst_hotels', $hotel);
 ...
 private function multiple_upload($upload_dir = 'uploads', $config = array())
         ...
         try {
            ...
            $hotel = array(
                ...
                'attachement_detail' => json_encode($hotel_files_details)
                 ...
                 $this->db->where('hotel_id', $id);
            $this->db->update('00000_mst_hotels', $hotel);
            return true;
    ...
    $id = $this->input->get('hotel_id');
    $this->db->where('hotel_id', $id);
    $this->db->delete('00000_mst_hotels');
    if($this->db->affected_rows() > 0){
      return true;
    }else{
      return false;
