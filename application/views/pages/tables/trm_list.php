    ...
    <div class="modal-content">
      ...
      <!-- <div class="modal-body">
          <form id="myForm" action="" method="post" class="form-horizontal">
            <input type="hidden" name="term_cond_id" value="0">
            <div class="form-group">
              <label for="name" class="label-control col-md-4">Branch Name</label>
              <div class="col-md-8">
                <input type="text" name="term_cond_desc" class="form-control">
              </div>
            </div>
             <div class="form-group">
                  <div class="col-md-8">
...
<script>
  $(function(){
    ...
    $('#btnAdd').click(function(){
      ...
      $('#myForm').attr('action', '<?php echo base_url() ?>term/addtrm');
    });
    ...
    $('#btnSave').click(function(){
      ...
      //validate form
      var term_cond_desc = $('input[name=term_cond_desc]');
      var result = '';
      if(term_cond_desc.val()==''){
        term_cond_desc.parent().parent().addClass('has-error');
      }else{
        term_cond_desc.parent().parent().removeClass('has-error');
        result +='1';
      }
    ...
    $('#showdata').on('click', '.item-edit', function(){
      var term_cond_id = $(this).attr('data');
      $('#myModal').modal('show');
      $('#myModal').find('.modal-title').text('Edit term Type');
      $('#myForm').attr('action', '<?php echo base_url() ?>term/updatetrm');
      $.ajax({
        type: 'ajax',
        method: 'get',
        url: '<?php echo base_url() ?>term/edittrm',
        data: {term_cond_id: term_cond_id},
        async: false,
        dataType: 'json',
        success: function(data){
          $('input[name=term_cond_desc]').val(data.term_cond_desc);
          $('input[name=term_cond_id]').val(data.term_cond_id);
    ...
    $('#showdata').on('click', '.item-delete', function(){
      ...
      $('#btnDelete').unbind().click(function(){
        $.ajax({
          ...
          method: 'get',
          async: false,
          url: '<?php echo base_url() ?>term/deletetrm',
          data:{term_cond_id:term_cond_id},
          dataType: 'json',
          success: function(response){
            if(response.success){
              $('#deleteModal').modal('hide');
              $('.alert-success').html('trm Type Deleted successfully').fadeIn().delay(4000).fadeOut('slow');
              showAlltrm();
            }else{
              alert('Error');
            }
    ...
    //function
    function showAlltrm(){
    var tableSrc = '<?php echo base_url() ?>term/showAlltrm';
