...
<!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Setup Data
        <small>Payment Terms List</small>
      </h1>
      <ol class="breadcrumb">
        <li><button class="btn btn-dropbox btn-sm"><a href="<?php echo base_url('payment'); ?>" style="color: whitesmoke">Add</a></button></li>
    ...
    <div class="modal-content">
      ...
      <!-- <div class="modal-body">
          <form id="myForm" action="" method="post" class="form-horizontal">
            <input type="hidden" name="pay_terms_id" value="0">
            <div class="form-group">
              <label for="name" class="label-control col-md-4">payment Type</label>
              <div class="col-md-8">
                <input type="text" name="pay_term_desc" class="form-control">
              </div>
            </div>
            <div class="form-group">
              <label for="address" class="label-control col-md-4">Notes</label>
              <div class="col-md-8">
...
<script>
  $(function(){
    ...
    $('#btnAdd').click(function(){
      $('#myModal').modal('show');
      $('#myModal').find('.modal-title').text('Add New payment Type');
    ...
    $('#btnSave').click(function(){
      ...
      if(pay_term_desc.val()==''){
        ...
        pay_term_desc.parent().parent().removeClass('has-error');
        result +='1';
      }
      if(address.val()==''){
    ...
    $('#showdata').on('click', '.item-edit', function(){
      var pay_terms_id = $(this).attr('data');
      $('#myModal').modal('show');
      $('#myModal').find('.modal-title').text('Edit payment Type');
      $('#myForm').attr('action', '<?php echo base_url() ?>payment/updatepayment');
      $.ajax({
        type: 'ajax',
        method: 'get',
        url: '<?php echo base_url() ?>payment/editpayment',
        data: {pay_terms_id: pay_terms_id},
        async: false,
        dataType: 'json',
        ...
        $.ajax({
          ...
          success: function(response){
            if(response.success){
              ...
              $('.alert-success').html('payment Type Deleted successfully').fadeIn().delay(4000).fadeOut('slow');
              showAllpayment();
            }else{
              alert('Error');
            }
    ...
    function showAllpayment(){
     var tableSrc = '<?php echo base_url() ?>payment/showAllpayment';
