...
//Including validation library
        ...
        $this->form_validation->set_rules('client_name', 'Client Name', 'required|min_length[1]|max_length[50]');

      $data['country'] = $this->country_model->fetch_country();
      $data['branches'] = $this->branch_model->fetch_branch();
      $data['sales'] = $this->saleman_model->fetch_sales();
      ...
      $data['travels'] = $this->travel_model->fetch_travel();
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            return $this->load->view('pages/forms/general', $data);
...
die;*/
            ...
            $this->client_model->client_insert($client);
            $client['message'] = 'Data Inserted Successfully';
             $client['country'] = $this->country_model->fetch_country();
              $client['branches'] = $this->branch_model->fetch_branch();
              $client['sales'] = $this->saleman_model->fetch_sales();
              $client['categories'] = $this->category_model->fetch_category();
              $client['hotels'] = $this->hotel_model->fetch_hotel();
              $client['payments'] = $this->payment_model->fetch_payment();
              $client['travels'] = $this->travel_model->fetch_travel();

            //loading view
             $this->load->view('header');
            $this->load->view('aside');
            //$this->load->view('nav_bar');
            $this->load->view('pages/forms/general', $client);
}
...
public function showAllclient(){
    $output = $this->client_model->showAllclient();
  /*  echo "<pre>";
    print_r($output);*/
/*     var_dump($output);die();*/
  ...
  public function addclient(){
    $result = $this->client_model->addclient();
    $msg['success'] = false;
    $msg['type'] = 'add';
    if($result){
      $msg['success'] = true;
  ...
  public function editclient(){
    $result = $this->client_model->editclient();
  ...
  public function updateclient(){
    ...
    $result = $this->client_model->updateclient();
    $msg['success'] = false;
    $msg['type'] = 'update';
    if($result){
      $msg['success'] = true;
    }
    header("Location: " . base_url('client/get_clients'));
  }


   public function deleteclient(){
    $msg['success'] = $this->client_model->deleteclient();
    echo json_encode($msg);
  }
      ...
      $data['travels'] = $this->travel_model->fetch_travel();
       // echo "This";
        $this->load->view('header');
            $this->load->view('aside');
            //$this->load->view('nav_bar');
            $this->load->view('pages/tables/data.php', $data);
    }
